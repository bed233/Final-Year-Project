...
public class AppScanner extends AsyncTask<Void, String, Void> {
    ...
    /**
     * Uses androidmanifest.xml to laod permissions needed for app to run and then analyse along with intents. Similar to APK Scanner however this focuses on already installed apps not APKs.
     * @param context
     * @param pkgName
     * @param scan_mode
     */
            ...
            if(scan_mode.equalsIgnoreCase("custom_scan")){
                ...
                if (prediction.equalsIgnoreCase(contextRef.get().getString(R.string.safe))) {
                    ...
                    notificationBuilder.setSmallIcon(R.drawable.ic_notification)
                            ...
                            .setPriority(NotificationCompat.PRIORITY_HIGH)
                            .addAction(R.drawable.libreav_ic_delete_notification, contextRef.get().getString(R.string.uninstall).toUpperCase(), uninstallPendingIntent);
                    notificationManager.notify(NOTIFICATION_ID, notificationBuilder.build());
                }
            }
        }
