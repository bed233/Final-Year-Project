...
suspend fun startup(context: Application) {
    ...
    prefs.edit().putInt(
        Common.RATING_DIALOG_APP_OPENING_COUNTER, prefs.getInt(
            Common.RATING_DIALOG_APP_OPENING_COUNTER, 0) + 1).apply()
    ...
    withContext(Dispatchers.IO) {
        ...
        File(Util.dataDir(context), "shared_prefs").listFiles { _, filename ->
            !Arrays.asList("com.google.android.gms.analytics.prefs.xml",
                    ...
                    xml(Common.PREFS_KEY), xml(
                    Common.PREFS_APPS), xml(
                    Common.MAXLOCK_PACKAGE_NAME), xml(
                    Common.PREFS_KEYS_PER_APP),
