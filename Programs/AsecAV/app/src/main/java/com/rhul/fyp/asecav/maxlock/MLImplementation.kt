...
package com.rhul.fyp.asecav.maxlock
...
import com.rhul.fyp.asecav.maxlock.daemon.MaxLockDaemon
...
object MLImplementation {
    ...
    fun getImplementation(prefs: SharedPreferences): Int {
        if (isDaemonRunning()) {
            prefs.edit().putInt(
                Common.ML_IMPLEMENTATION,
                DEFAULT
            ).apply() // Force DEFAULT if the daemon is already running
        }
        return prefs.getInt(
            Common.ML_IMPLEMENTATION,
            ACCESSIBILITY
    ...
    fun isActiveAndWorking(c: Context, prefs: SharedPreferences): Boolean {
        return (getImplementation(
            prefs
        ) == DEFAULT && isDaemonRunning()) || (getImplementation(
            prefs
